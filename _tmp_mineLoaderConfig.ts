/**
 * Mine-Loader / Voxel Realms ΓÇö fleet-owned hosts only.
 *
 ...
 *
 * Live SPA:  https://mine.grudge-studio.com  (CF ΓåÆ mine-loader.vercel.app)
 * API SSOT:  https://mine-loader-api-production.up.railway.app  (1 replica)
 * Open proxy: same-origin /api/blocks|/api/worlds ΓåÆ Railway (vercel.json)
 */
 ...
 * API is still Railway ΓÇö SPA rewrites /api/* on Vercel, or pass `api=` query.
 */
...
/** @deprecated Local staged SPA is not deployed on Vercel ΓÇö do not iframe. */
...
export const MINE_LOADER_PILLARS = [
  ...
  },
  {
    id: "combat",
    label: "Combat",
    blurb: "Melee, ranged, magic, armor ΓÇö fight wildlife, raiders, and bosses.",
  },
  {
    id: "adventure",
    label: "Adventure",
...
export interface MineLoaderLaunchOpts {
  ...
  /** Mine-Loader CHUNK_SIZES index 0..7 (16ΓÇª1024). */
  ...
  apiBase?: string | null;
  /**
   * @deprecated Ignored. Always uses fleet live host ΓÇö never /minegrudge or Replit.
   */
  preferLive?: boolean;
  forceLocal?: boolean;
}
 ...
 * CORS/SSO allowlist ΓÇö but production launches still prefer edge/Vercel.
 */
export function buildMineLoaderUrl(opts: MineLoaderLaunchOpts = {}): string {
  ...
  // Mine-Loader Vercel also rewrites /api/* ΓåÆ Railway; absolute API avoids drift.
  ...
  if (/\/minegrudge\//i.test(out)) {
    ...
    console.warn("[mineLoader] sanitized /minegrudge ΓåÆ", out);
  }
  return out;
}
