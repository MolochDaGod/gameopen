...
import { loadMouseFeel } from "../controlsSettings";
import {
  DIFFICULTY_COLOR,
  DIFFICULTY_SCALE,
  PROPS,
 ...
 *   - A deployable occupies the set of grid cells under its *footprint* at the
 *     level (`y`) it stands on. The footprint is derived from the deployable's
 *     normalized horizontal size: single-cell for NPCs / bags / the start
 *     marker, and the authored `PropDef.footprintRadius` for GLB props (so a
 *     wide "fortress" piece claims every cell it actually covers, known
 *     deterministically at click time).
 *   - A deployable may NEVER be buried: every footprint cell must be free of any
 *     terrain block AND free of any other deployable's footprint at that level.
 *     Invalid spots are refused (the click is a no-op) and the ghost turns red.
 *
 * In short: don't bury assets in maps/terrain/other assets — one footprint, one
 * owner, per level.
 */
...
export class VoxelEditor {
  ...
  private applyDeployTransform(group: THREE.Group, data: DeployableData): void {
    if (data.px !== undefined && data.py !== undefined && data.pz !== undefined) {
      ...
      group.position.set(data.x + 0.5, this.surfaceY(data.x, data.y, data.z), data.z + 0.5);
    }
  ...
  private surfaceY(x: number, y: number, z: number): number {
    for (let cy = y - 1; cy >= 0; cy--) {
      const b = this.blocks.get(cellKey(x, cy, z));
      if (b) return cy + shapeTopHeight(b.data.shape);
    }
    return 0;
  }
  ...
  /**
   * Whether a deployable of the given footprint can stand at cell (x,y,z): every
   * footprint cell must be free of terrain blocks AND of any other deployable's
   * footprint at the same level. `ignoreId` skips a deployable being replaced
   * (e.g. the existing start marker).
   */
  private deployPlacementValid(
    ...
    for (const fc of foot) {
      // Never bury a deployable inside a terrain block at its standing level.
      if (this.blocks.has(cellKey(fc.x, cell.y, fc.z))) return false;
      // Never overlap another deployable's footprint on the same level.
      for (const { data } of this.deployables.values()) {
        if (data.id === ignoreId) continue;
        if (data.y !== cell.y) continue;
  ...
  private placeDeployable(cell: { x: number; y: number; z: number }): void {
    ...
    if (!this.deployPlacementValid(cell, this.brushDeployRadius(), ignoreId)) return;
  ...
  private updateGhost(): void {
    ...
    const valid = this.deployPlacementValid(cell, radius, ignoreId);
