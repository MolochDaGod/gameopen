...
import { weaponCombat } from "../assets";
...
const BY_ID: Partial<Record<WeaponId, RangedPrimaryTune>> = {
  bow: BOW,
  crossbow: CROSSBOW,
  pistol: PISTOL,
  rifle: RIFLE,
...
export function rangedPrimaryTune(weaponId: WeaponId): RangedPrimaryTune {
  if (BY_ID[weaponId]) return { ...BY_ID[weaponId]! };
  try {
    const def = getWeapon(weaponId);
    if (def.group === "magic") return { ...STAFF };
    if (def.group === "ranged") return { ...RIFLE };
  } catch {
    /* ignore */
  }
  return { ...RIFLE };
 ...
 * Intensity mid (50) ≈ 1.0 multiplier.
 */
export function applyIntensity(
  tune: RangedPrimaryTune,
  weaponId: WeaponId,
): RangedPrimaryTune {
  const combat = weaponCombat(weaponId);
  const n = THREE.MathUtils.clamp(combat.intensity ?? 50, 1, 100) / 100;
  // Soft curve: low intensity slightly slower/weaker, high punches harder
