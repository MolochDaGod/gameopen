...
import { CHARACTER_HEIGHT_M, type CharacterDef, type Avatar } from "../types";
import { loadControls, loadMouseFeel } from "../controlsSettings";
...
export class EditorScene {
  ...
  constructor(container: HTMLElement, onChange: (s: EditorSnapshot) => void) {
    ...
    // Honor the studio-wide Mouse Sens so inspecting a rig here feels the same as
    // the Danger Room / Voxel Editor cameras rather than a fixed local speed.
    this.orbit.rotateSpeed = loadMouseFeel().sensitivity;
  ...
  private removeObject(o: EditorObject) {
    ...
    // A grudge playground character owns its own GPU resources + mixer.
    if (this.grudge.has(o.id)) {
  ...
  private weaponEditState(): EditorSnapshot["weaponEdit"] {
    ...
    if (id.startsWith("custom:")) {
      ...
      return { catalog: false, size: ov.size, pos: [...ov.pos], rot: [...ov.rot] };
    }
    ...
    return { catalog: true, size, pos: [pos[0], pos[1], pos[2]], rot: [rot[0], rot[1], rot[2]] };
  }
  ...
  async setWeaponSize(size: number): Promise<void> {
    ...
    const def = getWeapon(id);
    const tierDef = def.tiers?.[this.equippedTier];
    if (tierDef?.model) tierDef.model.length = v;
    else if (def.model) def.model.main.length = v;
    await this.mountCatalogModel();
  }
  ...
  /**
   * Enter Play mode: hand a loaded grudge character to the Animator's `Controller`
   * for 3rd-person WASD movement + mouse-look camera. Prefers the selected grudge
   * character, else the first one loaded. No-op (with a notice) if none exist.
   */
  startPlay(rootId?: string): void {
    if (this.playing) return;
    const pickId = rootId && this.grudge.has(rootId)
      ? rootId
      : this.selectedId && this.grudge.has(this.selectedId)
        ? this.selectedId
        : [...this.grudge.keys()][0];
    const grudge = pickId ? this.grudge.get(pickId) ?? null : null;
    // Prefer a spawned Playground character; otherwise drive the dressed rig so
    // Play mode always works on whatever character is on the stand.
    const driven: Avatar | null = grudge ?? this.rig;
    if (!driven) {
  ...
  private onPlayKeyDown = (e: KeyboardEvent) => {
    ...
    const vfxByDigit: Record<string, string> = {
      Digit1: "slashArc",
      Digit2: "nova",
      Digit3: "lightning",
      Digit4: "aoeBlast",
    ...
    const id = vfxByDigit[e.code];
    if (id) {
      // Turn to the aim and play the swing so the body matches the button, then
      // fire the now-directional effect.
      this.faceAim();
  ...
  private onPlayMouseDown = (e: MouseEvent) => {
    ...
    const avatar = this.driven;
    if (!avatar) return;
    this.faceAim();
    if (avatar.hasRole("attack")) avatar.playRoleOnce("attack", 0.1);
    this.playVfx("slashArc");
  };
  ...
  private emit() {
    ...
    this.onChange({
      ...
      grudgeChars: [...this.grudge.keys()].map((rootId) => ({
        rootId,
