...
export function CampfireLobby({ onExit, onNavigate, onAvatarEdit, onPlayDanger }: Props) {
  ...
  const bindHero = useCallback(
    (hero: GenesisHeroOption) => {
      gameSession.selectCharacter(hero.id);
      // Prefer Danger Room handoff path when provided (sets animator character id).
      if (onPlayDanger) {
        /* selection only — mode launch happens next */
      }
    },
    [onPlayDanger],
  );
    ...
    <div className="cfl-root">
      ...
      {panel === "shop" && (
        <DestPanel title="Shop" sub="Perks · frames · fleet store" onClose={() => setPanel(null)}>
          ...
          <div className="cfl-dest-grid">
            <button type="button" className="cfl-dest-btn" onClick={() => { setPanel(null); onNavigate("account"); }}>
