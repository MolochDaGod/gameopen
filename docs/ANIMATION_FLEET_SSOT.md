...
| **`mixamo-explorer`** | 25-bone Mixamo | Play: `/anims/baked/{pack}/*.json` rematch + `base/*.glb`. Author Mixamo `.fbx` is **local editor only** — not shipped on Vercel. | Explorer avatar, Open Danger `?era=voxel`, GRUDOX tvs-showcase, thrcc gun packs |
