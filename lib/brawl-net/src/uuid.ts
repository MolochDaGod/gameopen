/**
 * Runtime unique-id helper for Brawler entities (players, zombies, projectiles,
 * loot).
 *
 * IMPORTANT: this is the NON-deterministic counterpart to `rng.ts`.  Use it ONLY
 * for runtime identity — never for world generation, which must stay
 * seed-deterministic.  Mixing a uuid into world-gen would break reproducibility.
 */
